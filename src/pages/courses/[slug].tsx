...
const ICON_LIST: any = {
  BadgeCheckIcon: <BadgeCheckIcon className="h-6 w-6" aria-hidden="true" />,
  AcademicCapIcon: <AcademicCapIcon className="h-6 w-6" aria-hidden="true" />,
  ClockIcon: <ClockIcon className="h-6 w-6" aria-hidden="true" />,
  LibraryIcon: <LibraryIcon className="h-6 w-6" aria-hidden="true" />,
};
      ...
      {fourReasons?.map((reason: any) => (
        <div className="mt-8 sm:grid-cols-2" key={reason.id}>
          <span className="grid grid-flow-col gap-4">
            <BadgeCheckIcon className="h-8 w-8" aria-hidden="true" />
            {reason.description}
          </span>
        </div>
      ))}
...
export async function getStaticPaths() {
  return {
    paths: [
      { params: { slug: slugs.DCA } },
      { params: { slug: slugs.PYTHON1 } },
      { params: { slug: slugs.PYTHON2 } },
      { params: { slug: slugs.PYTHON3 } },
      { params: { slug: slugs.GST } },
      { params: { slug: slugs.TALLY } },
      { params: { slug: slugs.PGDCA } },
      // { params: { slug: slugs.ADCA } },
      // { params: { slug: 'pca' } },
